...
def collate_fn(processor: DetrImageProcessor, batch: List[Any], img_short: int, img_max: int):
...
def main():
    ...
    ap.add_argument("--outdir", default="weights/detr_coco_ft")
    ...
    if resume_ok:
        ...
        processor = DetrImageProcessor.from_pretrained(outdir)
        model = DetrForObjectDetection.from_pretrained(outdir).to(device)
    ...
    def set_backbone(mode: str):
        """
        mode:
          - "frozen": freeze whole backbone
          - "layer4": unfreeze only ResNet layer4 (best ‘Day2’ compromise)
          - "all": unfreeze full backbone
        """
    ...
    # ✅ 3) Load history.csv if it exists (so you don’t reset stats)
    ...
    if args.resume and hist_path.exists():
        ...
        if rows:
            # best_map should reflect best val_map50 so far (ignore epoch 0 baseline if you want)
    ...
    # ✅ One more important detail: backbone freeze state when resuming
    ...
    if args.resume and ckpt_path.exists():
        ...
        # ✅ If requested, force CLI LRs; otherwise keep checkpoint LRs
        if args.force_lr:
            # assuming param_groups[0] = non-backbone, param_groups[1] = backbone
    ...
    # ✅ 5) Don’t re-run baseline eval when resuming
    if not (args.resume and rows):
        ...
        # keep plots updated even when resuming
        write_history()

    # ✅ 4) Make your loop start from start_epoch
    for epoch in range(start_epoch, args.epochs + 1):
        # Day2 switch: unfreeze after N epochs (only triggers if you haven't passed it yet)
        if epoch == args.freeze_backbone_epochs + 1:
            set_backbone(args.unfreeze_mode)
            optimizer = build_optimizer()
            print(f"Unfroze backbone mode = {args.unfreeze_mode}")

            # When you rebuild optimizer, you should NOT lose scaler state.
            # (We keep scaler as-is.)
        ...
        # ✅ Save checkpoint every epoch (optimizer + scaler + progress)
        ckpt = {
            "epoch": epoch,
            "best_map": best_map,
    ...
    # Final test on best checkpoint you saved
